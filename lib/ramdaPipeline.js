...
var totalOrders = R.reduce(R.add, 0, R.map(x => x.amount, orders));
...
var cameraObjectByName = R.reduce(
    camera => {
        camera[0] = camera[0] || []
        camera[0].push({
            model: camera[1],
...
//console.log(JSON.stringify(cameraObjectByName(listOfCameras), null, 2));
